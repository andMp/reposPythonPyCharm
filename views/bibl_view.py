...
class bibl_view():
    ...
    def vsi_knigi(spis):
        if not spis:
            ...
            for i, kn in enumerate(spis, start=1):
                print(f'{i}. Kniga "{kn.nazva}", avtor: {kn.}, rik vidanna: {kn.rikVid}, ganr: {kn.ganr}, nayavnist: {'v biblioteci' if kn.stat==0 else 'na rukah'}, id knigi = {kn.id_kn}.')
    ...
    def per_kor(sp_chit, sp_kn):
        ...
        for i, ab in enumerate(sp_chit, start=1):
            print(f"{i}. Koristuvac: {ab.ima}, nomer chit. biletu: {ab.nomChitBil}, knigi: {', '.join(kn.nazva for kn in self.__spKnig if kn.idKn in ab.spisKn)}.")
